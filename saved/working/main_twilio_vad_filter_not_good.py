...
import numpy as np
import audioop
from  audio_segmentation import detect_silent_zones

load_dotenv(override=True)

# Initialize global audio buffer
stream_input = np.array([])
...
async def handle_media_stream(websocket: WebSocket):
    ...
    global stream_input  # Access the global buffer
    
    # Initialize stream_input for this connection
            ...
            try:
                async for message in websocket.iter_text():
                    ...
                    if data['event'] == 'media':
                        ...
                        # Store raw PCM bytes in persistent buffer
                        global stream_input
                        # Convert PCM to numpy array and concatenate with existing array
                        pcm_np = np.frombuffer(pcm_data, dtype=np.int16)
                        stream_input = np.concatenate([stream_input, pcm_np]) if stream_input.size > 0 else pcm_np
                        ...
                        if ulaw_file is None:
                            ...
                            audio_append = {
                                ...
                                "audio": base64.b64encode(ulaw_data).decode('utf-8')
                            }                            

                        await openai_ws.send(json.dumps(audio_append))

                        # Use the numpy array directly for silence detection
                        if not llm_speaking and detect_silent_zones(stream_input):

                            print()
                            print("Silent zone detected, sending response.")
                            print("Silent zone detected, sending response.")
                            print()
                            ...
                            stream_input.clear()  # Clear the global buffer
